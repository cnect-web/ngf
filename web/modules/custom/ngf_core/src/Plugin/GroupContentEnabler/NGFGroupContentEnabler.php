 ...
 *
 * @NGFGroupContentEnabler(
 *   id = "ngf_group_membership",
 *   label = @Translation("NGF Group membership"),
 ...
 * )
 */
class NGFGroupContentEnabler extends GroupContentEnablerBase {
  ...
  public function postInstall() {
    ...
    if (!\Drupal::isConfigSyncing()) {
      ...
      // Add the group_membership_state field to the newly added group content type. The
      // field storage for this is defined in the config/install folder. The
      // default handler for 'group_membership_state' target entities in the 'group_type'
      // handler group is GroupTypeMembershipStateSelection.
      FieldConfig::create([
        'field_storage' => FieldStorageConfig::loadByName('group_content', 'group_membership_state'),
        'bundle' => $group_content_type_id,
        'label' => $this->t('State'),
        'settings' => [
          'handler' => 'group_type:group_membership_state',
          'handler_settings' => [
            'group_type_id' => $this->getGroupTypeId(),
          ],
        ],
      ...
      // Assign widget settings for the 'default' form mode.
      $form_display->setComponent('group_membership_state', [
        'type' => 'options_select',
      ])->save();

      // Assign display settings for the 'default' view mode.
      $view_display->setComponent('group_membership_state', [
        'label' => 'above',
        'type' => 'entity_reference_label',
        'settings' => [
          'link' => 0,
